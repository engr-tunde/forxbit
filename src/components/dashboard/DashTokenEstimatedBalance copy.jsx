...
const DashTokenEstimatedBalance = ({
  ...
  // let token_bal_value =
  //   token_price_value &&
  //   Number(selectedToken.available_balance * token_price_value).toFixed(2);
    ...
    if (tokenBalances) {
      async function getBals() {
        ...
        for (let ele in arrData) {
          console.log("arrData[ele]?.ticker", arrData[ele]);
          // await fetch(
          ...
          try {
            const res = await fetch(
              `${import.meta.env.VITE_CC_IPA_ESAB}/coins/${selectedToken?.id}`,
              {
                method: "GET",
                headers: CG_HEADERS,
              }
            ...
            response = response?.market_data?.current_price;
            token_price = Object.values(response)[0];
            token_bal_val = Number(
              arrData[ele].available_balance * token_price
            );
            ...
            if (ele == Number(arrData.length) - 1) {
              try {
                ...
                const res2 = await fetch(
                  `${import.meta.env.VITE_CC_IPA_ESAB}/coins/${
                    selectedToken?.id
                  }`,
                  {
                    method: "GET",
                ...
                let response2 = await res2.json();
                response2 = response2?.market_data?.current_price;
  ...
  //           token_price = Object.values(response)[0];
  //           token_bal_val = Number(arrData[ele].available_balance * token_price);
  //           tokenVal = tokenVal + token_bal_val;
  //           if (ele == Number(arrData.length) - 1) {
